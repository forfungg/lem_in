...
class Node():
	...
	def add_neighbor(self, v):
		if v != self.name and v not in self.neighbors:
			self.neighbors[v] = 1
...
class Colony():
	...
	def ford_fulkerson(self):
		limit = min(len(self.nodes[self.start].neighbors), self.ants)
		print(limit)
		q = [[self.start]]
		while q:
			path = q.pop(0)
			...
			if node.name == self.end:
				continue
			for n in node.neighbors:
				if node.neighbors[n] > 0 and n != self.start and not n in path:
					new = list(path)
					new.append(n)
					q.append(new)
					node.neighbors[n] = 0
					self.nodes[n].neighbors[node.name] = 1
...
if __name__ == "__main__":
	my_colony = Colony()
	with open("../../resources/example06", "r") as f:
		ret = f.read()
